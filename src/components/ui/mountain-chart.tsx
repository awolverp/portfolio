...
function generateMountain(width: number, height: number, seed: number): Point[] {
...
function readVar(el: HTMLElement, probe: CanvasRenderingContext2D, name: CssVar) {
...
export function MountainChart({ className, colors = defaultColors }: MountainChartProps) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const trail = slice(mapped, cum, Math.max(0, lightLen - pathLen * TRAIL), lightLen);
