...
export function DesignDatabaseStage() {
  return (
    <Stage.Root>
      <Stage.Layer position="top" className="grid grid-cols-4 place-items-end z-20">
      ...
      <Stage.Layer position="bottom" className="grid grid-cols-4 place-items-start z-20">
...
function ColumnRow({ column }: { column: (typeof tables)[number]["columns"][number] }) {
  return (
...
function ColumnIcon({ kind }: { kind: "pk" | "fk" | "column" }) {
  ...
  return <CircleSmallIcon className="size-3.5 shrink-0 text-muted-foreground" />;
}
