...
export function EnforceInvariantsStage() {
  return (
    <Stage.Root>
      ...
      <Stage.Layer position="bottom" className="flex min-h-0 w-full justify-center px-4">
        <TaskList />
      </Stage.Layer>
    </Stage.Root>
  );
...
const listStyle = cn("relative flex h-full w-full max-w-3xl min-w-0 flex-col gap-2.5");
    ...
    >
      <LayoutGroup>
        {state.pending.map((item) => (
          <TaskRow key={item.key} item={item} done={false} checked={state.checking === item.key} />
        ))}
        <DoneDivider />
        {state.done.map((item) => (
...
function TaskRow({ item, done, checked }: { item: TaskItem; done: boolean; checked: boolean }) {
  return (
    <m.div
      layout={true}
      layoutId={item.key}
