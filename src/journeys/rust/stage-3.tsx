...
function FloatingPhrases({ active }: { active: boolean }) {
  ...
  const take = (release: string) => {
    ...
    const next = available[Math.floor(Math.random() * available.length)] ?? release;
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-0">
      {phrases.slice(0, SLOT_COUNT).map((phrase, index) => (
        <PhraseSlot key={phrase} index={index} initialPhrase={phrase} active={active} take={take} />
      ))}
    </div>
  );
}
...
function pickPosition() {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (left < exclude.left || left > exclude.right || top < exclude.top || top > exclude.bottom) {
      return { left, top };
    }
  }
...
function PhraseSlot({
  ...
  return (
    <m.span
      ...
      animate={active ? { opacity: [0, 1, 1, 0], y: [12, 0, 0, -8] } : { opacity: 0, y: 0 }}
      transition={
        active
          ? {
