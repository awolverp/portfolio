...
const projectTypeSchema = z.enum(["contract", "freelance", "personal", "open-source", "full-time"]);
...
export function selectSkillsForJourney(config: Config, journey: Journey): SkillItem[] {
  let cached = journeySkillsCache.get(config);
  if (!cached) {
    const items = config.resume.skills.flatMap((group) => group.items);
    cached = {
      "full-stack": items.filter((item) => item.journeys?.includes("full-stack")),
...
export function parseConfig(raw: Config): Config {
