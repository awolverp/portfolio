...
function EndpointRow({ endpoint }: { endpoint: (typeof endpoints)[number] }) {
  return (
    <div className={rowStyle}>
      <span className={methodVariants({ method: endpoint.method })}>{endpoint.method}</span>
      <span className="shrink-0 font-mono text-muted-foreground">{endpoint.path}</span>
      <span className="min-w-0 truncate text-muted-foreground">{endpoint.name}</span>
